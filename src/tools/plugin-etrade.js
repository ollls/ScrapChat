...
export default {
  ...
  prompt: `## E*TRADE FINANCIAL DATA (Precision Mode rules above apply to all data below)
...
- To save E*TRADE data for Python analysis — use etrade_account with "saveAs" parameter (e.g. saveAs: "transactions.csv"). NEVER use run_python to fabricate E*TRADE data — only etrade_account has real data.
...
- QUOTES: Always use detailFlag "ALL" for quotes unless the user specifically asks for a narrower view. "WEEK_52" omits bid/ask/change/P\\/E — don't use it and then complain about missing fields.
...
- OPTION CHAIN FETCHING: For single-expiry requests or when the user asks for a specific delta range, covered calls, or any filtered view — fetch the FULL chain (omit noOfStrikes, omit strikePriceNear). Do NOT crawl through multiple strikePriceNear values — that wastes rounds. Only use strikePriceNear + noOfStrikes when the user explicitly requests a narrow range around a specific strike.
