...
import { listCompacts, upsertCompact, updateCompact, deleteCompact, reorderCompacts } from '../services/compacts.js';
...
async function generateTitle(text) {
  try {
    ...
    console.warn('generateTitle (compact): no usable content. raw:', JSON.stringify(raw));
  } catch (err) {
    console.warn('generateTitle (compact) failed:', err.message, err.cause || '');
  }
...
router.post('/', async (req, res) => {
  ...
  res.json(upsertCompact(color.trim(), text.trim(), title));
});
...
router.patch('/:id', (req, res) => {
  ...
  const compact = updateCompact(req.params.id, updates);
...
router.put('/reorder', (req, res) => {
  ...
  res.json(reorderCompacts(ids));
});

router.delete('/:id', (req, res) => {
  deleteCompact(req.params.id);
  res.json({ ok: true });
});
